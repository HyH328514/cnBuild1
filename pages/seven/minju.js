...
function initMaterialPieChart() {
    ...
    // 明清时期民居建筑材料占比数据
    let data = [
        { value: 40, name: '土体 / 夯土 / 土坯' },
        { value: 30, name: '木材（构架、楼板、门窗）' },
        { value: 16, name: '砖瓦（青砖、青瓦）' },
        { value: 8, name: '灰浆（石灰、砂、粘结料）' },
        { value: 5, name: '石材（台基、柱础、墙角）' },
        { value: 1, name: '铁件、辅料、竹材等' }
    ];
        ...
        series: [
            {
                ...
                // 模拟数据（可根据实际需求调整）
                data: [
                    { value: 35, name: '抬梁式木架构' },
                    { value: 65, name: '穿斗式木架构' }
                ]
            }
        ]
    };
...
function initStructureCharts(type) {
    ...
    const hemuduBarData = {
        title: '抬梁式木架构民居结构参数',
        xAxis: ['柱网间距', '木材消耗', '室内空间', '屋顶坡度'],
        yAxis: [50, 40, 50, 20] // 平均数据
    };
    const banpoBarData = {
        title: '穿斗式木架构民居结构参数',
        xAxis: ['柱网间距', '木材消耗', '室内空间', '屋顶坡度'],
        yAxis: [20, 20, 25, 45] // 平均数据
    };
    const hemuduPieData = [
        { value: 38, name: '木材（大梁、粗柱、檩椽）' },
        { value: 35, name: '砖瓦（厚砖墙、屋面）' },
        { value: 18, name: '夯土 / 土坯' },
        { value: 3, name: '灰浆辅料' },
        { value: 6, name: '石材（台基、柱础）' }
    ];
    const banpoPieData = [
        { value: 52, name: '木材（密柱、穿枋、檩条）' },
        { value: 22, name: '砖瓦（薄墙、山墙、屋面）' },
        { value: 12, name: '竹材 / 板材 / 围护' },
        { value: 5, name: '灰浆辅料' },
        { value: 9, name: '石材（基础、天井）' }
    ];
    ...
    const pieOption = {
        title: {
            text: type === 'hemudu' ? '抬梁式木架构民居组成' : '穿斗式木架构民居组成',
            left: 'right'
        },
        ...
        series: [
            {
                ...
                // 明清民居功能数据（贴合文本描述）
                data: [
                    { value: 55, name: '居住起居（堂屋、卧房、厢房）' },
                    { value: 20, name: '交通过渡（天井、巷道、廊檐、过道）' },
                    { value: 12, name: '辅助生活（厨房、柴房、杂屋）' },
                    { value: 8, name: '仓储畜养（粮仓、库房、畜圈）' },
                    { value: 5, name: '礼仪祭祀（祖堂、香火位）' },
