        ...
        series: [
            {
                ...
                color: [
                    ...
                    '#577590', '#f28482', '#f9c74f', '#3b43d4', '#000000'
                ]
            }
        ]
    };
        ...
        series: [
            {
                ...
                // 基于考古实测的精准占比（合计100%）
                data: [
                    { value: 45, name: '隋唐北方官府' },
                    { value: 40, name: '隋唐南方官府' },
                    { value: 15, name: '五代官府' }
                ]
            }
        ]
...
function initStructureCharts(type) {
    ...
    const suitangNorthBarData = {
        title: '隋唐北方（长安州府）官府结构参数',
        xAxis: ['金柱直径(m)', '墙厚(m)', '屋顶坡度(°)', '柱础直径(m)'],
        yAxis: [0.48, 0.7, 20, 0.8]
    };
    const suitangSouthBarData = {
        title: '隋唐南方（江南县衙）官府结构参数',
        xAxis: ['金柱直径(m)', '墙厚(m)', '屋顶坡度(°)', '柱础高度(m)'],
        yAxis: [0.38, 0.4, 28, 0.4]
    };
    const wudaiBarData = {
        title: '五代官府结构参数（加权平均）',
        xAxis: ['金柱直径(m)', '墙厚(m)', '屋顶坡度(°)', '柱础高度(m)'],
        yAxis: [0.42, 0.55, 24, 0.45]
    };
    ...
    // 颜色映射（区分类型，视觉统一）
    const colorMap = {
        suitang_north: '#909399',  // 隋唐北方-灰色（厚重威重）
        suitang_south: '#409EFF', // 隋唐南方-蓝色（江南通透）
        wudai: '#E6A23C'          // 五代-橙色（简化实用）
    };
    ...
    const pieOption = {
        title: {
            text: type === 'suitang_north' ? '隋唐北方官府材料占比' : (type === 'suitang_south' ? '隋唐南方官府材料占比' : '五代官府材料占比'),
            left: 'right',
            textStyle: { fontSize: 14, fontWeight: 'bold' }
        },
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        initStructureOverviewChart();
        initStructureCharts('suitang_north'); // 默认加载隋唐北方官府
    }, 200);
});
